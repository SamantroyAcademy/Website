    ...
    <>
      ...
      <div ref={bar} className="fixed inset-x-0 bottom-0 z-40 border-t border-line bg-paper/95 pb-[env(safe-area-inset-bottom)] backdrop-blur-xl md:hidden">
        <nav className="flex items-stretch" aria-label="Quick actions">
          ...
          <button type="button" onClick={() => open()} className="m-1.5 flex flex-[1.3] items-center justify-center gap-1.5 rounded-full bg-accent text-[0.8rem] font-semibold text-white active:scale-95 transition-transform">
      ...
      <div className="fixed bottom-6 right-6 z-40 hidden flex-col items-end gap-3 md:flex">
        ...
        <a
          ...
          className="group flex h-14 items-center overflow-hidden rounded-full bg-brand-700 px-4 text-sm font-semibold text-surface shadow-[0_14px_30px_-12px_rgb(12_17_36/0.6)] transition-colors hover:bg-brand-600"
        >
