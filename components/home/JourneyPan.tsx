...
function StageCard({ s }: { s: JourneyStage }) {
  return (
    <article className="card relative flex h-full flex-col overflow-hidden p-6 sm:p-7">
      ...
      <div className="rich-html mt-3 text-[0.95rem] leading-relaxed text-ink-2" dangerouslySetInnerHTML={{ __html: s.brief }} />
      {s.tests?.length > 0 && (
      ...
      {s.drill && (
        <div className="mt-auto pt-6">
          <div className="rounded-[14px] bg-brand-50 px-4 py-3.5">
            ...
            <div className="rich-html mt-1 text-sm leading-relaxed text-brand-900" dangerouslySetInnerHTML={{ __html: s.drill }} />
          </div>
        </div>
      )}
    </article>
...
export default function JourneyPan({ stages, intro }: { stages: JourneyStage[]; intro: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Desktop: vertical scroll pans the stages horizontally while pinned.
    ...
    // Mobile: the timeline line draws as you read down.
  ...
  return (
    <section ref={wrap} className="journey-wrap relative overflow-hidden bg-tint py-20 lg:flex lg:h-[100dvh] lg:items-center lg:py-0" aria-label="Recruitment journey">
      {/* Desktop track */}
      <div ref={track} className="journey-track hidden gap-5 pl-[max(2rem,calc((100vw-1320px)/2+2rem))] pr-16 lg:flex lg:items-stretch">
        {intro && <div className="w-[26rem] shrink-0 py-2 pr-6">{intro}</div>}
        {stages.map((s, i) => (
          <div key={s.code + i} className="h-[min(42rem,84dvh)] w-[25rem] shrink-0">
            <StageCard s={s} />
          </div>
        ))}
      </div>
