 ...
 *  same frame (privacy-enhanced domain, no autoplay before the tap). */
      ...
      {playing ? (
        ...
        <button type="button" onClick={() => setPlaying(true)} className="group absolute inset-0 text-left" aria-label={`Play: ${title}`}>
          ...
          <span className="absolute inset-x-0 bottom-0 p-4">
            <span className="line-clamp-3 text-[0.92rem] font-semibold leading-snug text-white">{title}</span>
          </span>
        </button>
      )}
    </div>
